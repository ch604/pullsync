sshkeygen() { #generate a unique ssh key and copy it to the source server.
	...
	if [ "$autopilot" ]; then
		...
		for file in /root/.ssh/pullsync*.pub; do
			[ -f "$file" ] && rm -f /root/.ssh/pullsync* && break
		done
		...
		sssh "mkdir -p -m600 ~/.ssh; cat >> ~/.ssh/authorized_keys; chmod 600 ~/.ssh/authorized_keys" < ~/.ssh/"$keyname".pub
	fi

	# ensure that the conneciton works
	ec yellow "Testing ssh connection..."
	if ! sssh -oConnectTimeout=10 "true" ; then
	...
	# append ssh args to not use the password auth method now that the key is set up; causes pullsync to fail more quickly if theres a key problem
	sshargs="$sshargs -oPasswordAuthentication=no"
	...
	sssh "which firewall && firewall stop" &> /dev/null
