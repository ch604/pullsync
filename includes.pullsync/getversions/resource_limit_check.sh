resource_limit_check() { # determine if any oom, maxconn, or mysql 'too many connections' limits were hit per logs.
	...
	if [ "$oom_hits" ]; then
		ec red "Source server ran OOM killer in the past!" | errorlogit 3
		echo "$oom_hits" | tee -a $dir/resource_oom.txt | tail -n10
		say_ok
	fi
	if [ "$mc_hits" ]; then
		ec red "Source server hit MaxClients in the past!" | errorlogit 3
		echo "$mc_hits" | tee -a $dir/resource_mc.txt | tail -n10
		say_ok
	fi
	if [ "$cnxn_hits" ]; then
		ec red "Source server hit MaxConnections for mysql in the past!" | errorlogit 3
