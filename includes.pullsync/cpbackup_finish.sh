cpbackup_finish() { #if enabled in backupcheck(), toggle on backups for all users
	...
	for user in $userlist; do
		...
		[ ! $(/usr/local/cpanel/bin/whmapi1 accountsummary user=$user | awk '/ backup:/ {print $2}') -eq 1 ] && /usr/local/cpanel/bin/whmapi1 toggle_user_backup_state user=$user 2>&1 | stderrlogit 3
	done
}
