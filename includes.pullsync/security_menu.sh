security_menu() { #run outside of matching_menu() in case version matching is not needed, but security options are wanted
	local choices modsecon
	declare -a cmd options
	cmd=(dialog --clear --backtitle "pullsync" --title "Security Menu" --separate-output --checklist "Select options for server security. Sane options were selected based on your configuration:\n" 0 0 6)
	options=( 1 "Enable OWASP mod_security ruleset (fight intrusion)" off
	...
	if rpm --quiet -q ea-apache24-mod_reqtimeout; then
		unset "options[8]" "options[7]" "options[6]"
	...
	#OWASP (0 1 2)
	if [ $modsecon ]; then
		unset "options[2]" "options[1]" "options[0]"
