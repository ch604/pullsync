wpt_speedtest() { #actually perform the webpagespeed test. test domain $1 using live dns, or optionally against IP $2 (usage: wpt_speedtest domain.com [123.45.67.89])
	local testserver loc wptargs testid csvfile wptloop medianttfb medianlt result
	declare -a lettergrades
	# specify server and location, and variables to use for the test
	testserver="https://www.webpagetest.org"
	loc="ec2-us-east-1-catchpoint"
	wptargs="--data runs=3 --data noimages=1 --data ignoreSSL=1 --data location=$loc --data keep_test_private=1 --data video=0"

	# if there is an IP specified, use it, otherwise use live dns
	if [ "$2" ]; then
		ec yellow "Starting WPT against $1 using $2..."
		testid=$(curl -kvs $testserver/runtest.php --data script=setDns+$1+$2%0D%0AsetDns+www.$1+$2%0D%0Anavigate+$1 $wptargs 2>&1 | grep ^\<\ Location\: | cut -d\= -f2 | sed 's/\r//')
	else
		ec yellow "Starting WPT against $1 using live DNS..."
		testid=$(curl -kvs $testserver/runtest.php --data url=$1 $wptargs 2>&1 | grep ^\<\ Location\: | cut -d\= -f2 | sed 's/\r//')
	fi
	...
	if [ $testid ]; then
		...
		medianttfb=$(awk -F, '{print $3}' $csvfile | sort | awkmedian)
		medianlt=$(awk -F, '{print $2}' $csvfile | sort | awkmedian)
		# score the ttfb
		...
		for col in 4 5 6 8 9; do
			...
			result=$(awk -F, '{print $'$col'}' $csvfile | sort | tail -1)
