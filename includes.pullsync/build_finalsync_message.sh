build_finalsync_message() { #create a ticket reply for post-finalsync based on server and nameserver information, including sync-specific information
	local testuser testdomain nameserver1 nameserver2 snameserver1 snameserver2 first_ip second_ip registrar techcontact choice
	declare -a cmd options
	cmd=(dialog --nocancel --colors --clear --backtitle "pullsync" --title "Build Final Sync Message" --radiolist "Select a nameserver hosting option for building the post-final message to the client.\n" 0 0 7)
	options=( 1 "Internal to source (and customer will reuse same nameservers)" off
	...
	testuser=$(awk '{print $1}' <<< "$userlist")
	testdomain=$(awk -F= '/^DNS=/ {print $2}' "/var/cpanel/users/$testuser")
	nameserver1=$(awk '/^NS / {print $2}' /etc/wwwacct.conf)
	nameserver2=$(awk '/^NS2 / {print $2}' /etc/wwwacct.conf)
	snameserver1=$(awk '/^NS / {print $2}' "$dir/etc/wwwacct.conf")
	snameserver2=$(awk '/^NS2 / {print $2}' "$dir/etc/wwwacct.conf")
	...
	if [ -f /var/cpanel/cpnat ] && grep -Eq "^$first_ip [0-9]+" /var/cpanel/cpnat; then
		first_ip=$(awk '/^'"$first_ip"' [0-9]+/ {print $2}' /var/cpanel/cpnat)
	fi
	if [ -f /var/cpanel/cpnat ] && grep -Eq "^$second_ip [0-9]+" /var/cpanel/cpnat; then
		second_ip=$(awk '/^'"$second_ip"' [0-9]+/ {print $2}' /var/cpanel/cpnat)
	fi

	#use the nameserver registrar and contact information to guide suggestion
	registrar=$(nameserver_registrar "$nameserver1")
	...
	elif head -2 "$dir/dnsoutput/nameserver_summary.txt" | grep -q "$nameserver1" && head -2 "$dir/dnsoutput/nameserver_summary.txt" | grep -q "$nameserver2"; then
		if [ ! "$copydns" ]; then
			options[8]=on
	...
	elif ! head -2 "$dir/dnsoutput/nameserver_summary.txt" | grep -q -e "$nameserver1" -e "$nameserver2" -e "$snameserver1" -e "$snameserver2"; then
		options[8]=on
