oldmigrationcheck() { #always run during synctype_logic, to get old ip/port and pre-sync the config folder
	...
	# list the pullsync folders on the server
	...
	if [ ${#oldlist[@]} = 0 ]; then
		...
		for each in ${oldlist[@]}; do
			...
			tempoldcount=`cat /home/temp/$each/userlist.txt | wc -w`
			options+=($i "${each#*.} \Z2$tempoldip:$tempoldport\Zn count: \Z2$tempoldcount\Zn" off)
			let i+=1
		done
