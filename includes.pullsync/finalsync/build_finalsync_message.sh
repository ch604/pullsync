build_finalsync_message() { #create a ticket reply for post-finalsync based on server and nameserver information, including sync-specific information
	...
	local options=( 1 "Internal to source (and customer will reuse same nameservers)" off
			2 "Internal to source, but registered by LiquidWeb" off
			3 "Customer will use brand new nameservers on new server" off
			4 "ns.liquidweb.com" off
			5 "External to source (e.g. GoDaddy, Cloudflare, etc.)" off
			6 "Other (add your own information when copy/pasting)" off
			7 "IP swap, doesn't matter" off)
	...
	if [ "$registrar" = "ENOM, INC." ]; then
		techcontact=$(whois $(echo $nameserver1 | cut -d. -f2-) | awk -F: '/^Tech Organization:/ {print $NF}' | tr -d ' ')
		if echo $techcontact | grep -q -i liquidweb; then
			cmd[9]=$(echo "${cmd[9]}\n\Z1${nameserver1}'s main domain seems to be registered by liquidweb!\nPlease remember to update this at the conclusion of the DNS changes if needed!\Zn\n")
		elif echo $techcontact | grep -q -i whoisguard; then
			cmd[9]=$(echo "${cmd[9]}\n\Z1${nameserver1}'s main domain seems to be registered at enom, but whoisguard is on.\nThis could be registered by liqudweb! double check in billing before making a selection!\Zn\n")
		else
	...
	#create suggestion
	# per #139, disabled ip swap temporarily, but added "dummyipswap" variable to suggest ip swap verbiage.
	if [ "$ipswap" -o "$stormipswap" -o "$dummyipswap" ]; then
		options[20]=on
		cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 7, ipswap variable is set.")
	elif head -2 $dir/nameserver_summary.txt | grep -q $nameserver1 && head -2 $dir/nameserver_summary.txt | grep -q $nameserver2; then
		if [ ! "$copydns" ]; then
			options[14]=on
			cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 5, top nameservers are those set on this machine, but you elected to not copy DNS.")
		elif echo $techcontact | grep -q -i -e liquidweb -e whoisguard; then
			options[5]=on
			cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 2, nameservers from /etc/wwwacct.conf match top two used nameservers, registration possibly by liquidweb.")
		else
			options[2]=on
			cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 1, nameservers from /etc/wwwacct.conf match top two used nameservers.")
		fi
	elif head -2 $dir/nameserver_summary.txt | grep -q -e liquidweb.com -e sourcedns.com -e wiredtree.com; then
		options[11]=on
		cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 4, top used nameservers are liquidweb, wiredtree, or sourcedns.")
	elif ! head -2 $dir/nameserver_summary.txt | grep -q -e $nameserver1 -e $nameserver2 -e $snameserver1 -e $snameserver2; then
		options[14]=on
		cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 5, top used nameservers are neither source or target's.")
	elif ! grep ^NS $dir/etc/wwwacct.conf | grep -q -e $nameserver1 -e $nameserver2; then
		options[8]=on
		cmd[9]=$(echo "${cmd[9]}\nI recommend picking option 3, nameservers do not match between source and target.")
	else
...
The final sync is complete, and the DNS files for the domains have been synced back to the original server. We have also updated the nameserver IPs for you at the domain registrar, since your nameservers' primary domain was registered through us. You can see that the updates are public and propagated by using this tool:

https://www.whatsmydns.net/#A/$testdomain

DNS for any domains that do not depend on your internal nameservers should be updated at this time. With this, the migration can be considered fully complete.
EOF
			;;
...
EOF
			...
			;;
			4) cat >> $dir/finalsyncreply.txt << EOF
The final sync is complete, and the DNS for the domains has been updated on our nameservers, ns.liquidweb.com. This has made the sites live on the new server. You can see that the updates are public and propagated by using this tool:

https://www.whatsmydns.net/#A/$testdomain

DNS for any domains that do not depend on our Liquid Web nameservers should be updated at this time. With this, the migration can be considered fully complete.
...
https://www.liquidweb.com/products/add-ons/storage-backups/acronis-cyber-backups/
...
EOF
	...
	# detect if lw source, and if so storm/dedi; give termination strategy
	if [ -f $dir/usr/local/lp/etc/lp-UID ]; then
		if sssh "df | awk '{print $1}'" | grep -qE 'vda[0-9]'; then
			cat >> $dir/finalsyncreply.txt << EOF
Keep in mind that we will not automatically cancel the old hosting solution for you; this must be requested in a separate ticket once you are sure you no longer need data from that machine. A backup archive of the server is recommended in cases when long term retention may be needed, which could be accomplished through a Storm Image:
...
EOF
			...
			cat >> $dir/finalsyncreply.txt << EOF
Keep in mind that we will not automatically cancel the old hosting solution for you; this must be requested in a separate ticket once you are sure you no longer need data from that machine.
