install_parallel() { #build parallel and set will-cite to keep commands unattended
	...
	if [ ! `which parallel 2> /dev/null` ]; then
		...
		# get the latest public version if ftp.gnu.org is up, otherwise use github
		if (timeout 1 bash -c 'echo > /dev/tcp/ftp.gnu.org/80') &> /dev/null; then
			...
			wget -q http://raw.githubusercontent.com/ch604/pullsync/master/parallel-latest.tar.bz2 -O parallel-latest.tar.bz2
		fi
