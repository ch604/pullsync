initialsync_main() { #the meaty heart of pullsync. performs the pre and post migration tasks for initially syncing accounts, including calling version matching menus and commands and cleanup and post-sync commands. this houses the breaking point between the attended and unattended sections.
	...
	if  [ "$synctype" == "list" ] || [ "$synctype" == "domainlist" ] || [ "$synctype" == "all" ];then # no single or skeletons
		...
		if [ ! "$autopilot" ] ; then
			...
			# modsec versions fuzzy match if local whitelist empty, remote whitelist has size
			[ "$(echo $localmodsec | cut -d- -f1-3)" = "$(echo $remotemodsec | cut -d- -f1-3)" ] && [[ "$localmodsec" =~ "lp-modsec2-rules" ]] && [ ! -s /usr/local/apache/conf/modsec2/whitelist.conf -a ! -s /etc/apache2/conf.d/modsec2/whitelist.conf ] && [ -s $dir/usr/local/apache/conf/modsec2/whitelist.conf -o -s $dir/etc/apache2/conf.d/modsec2/whitelist.conf ] && modsecimport=1
			do_optimize=1
			optimize_auto
			security_auto
		fi
