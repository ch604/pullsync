optional_items_menu(){ #additional options for initial syncs
	...
	local options=(	1 "Comment out user crons during restore" on
			...
			3 "Run marill automated testing after initial sync" on
			4 "Add motd line for in-progress migration" on
			5 "Use --dbbackup=schema for pkgacct" off
			6 "Run fixperms for migrated accounts" off
			7 "Convert Roundcube MySQL to sqlite on source" off)
...
for ip in data["ip"]:
 if ip["mainaddr"] == 0:
  print ip["used"]' | grep 0 | wc -l`
	...
	#marill (6 7 8)
	[ "$synctype" = "skeletons" ] && options[8]=off && cmd[8]=`echo "${cmd[8]}\n(3) No marill needed on skeleton syncs"`

	#motd (9 10 11)
	grep -q pullsync /etc/motd && options[11]=off && cmd[8]=`echo "${cmd[8]}\n(4) motd for pullsync already exists"`

	#dbbackup schema (12 13 14)
	sssh "/scripts/pkgacct --help | grep -q -e --dbbackup=" && options[14]=on && cmd[8]=`echo "${cmd[8]}\n(5) Source server supports dbbackup argument"`

	#fixperms (15 16 17)
	for user in $userlist; do
		[[ ! "$(sssh "stat /home/$user/public_html" | grep Uid | awk -F'[(|/|)]' '{print $2, $6, $9}')" =~ 751\ +$user\ +nobody ]] && local fixmatch=1
	done
	[ $fixmatch ] && cmd[8]=`echo "${cmd[8]}\n(6) Some accounts have incorrect public_html permissions (you still need to turn this on if you want to run fixperms)"` && unset fixmatch

	#rcube (18 19 20)
	[ ! "$synctype" = "single" ] && sssh "mysql -Nse 'show databases'" | grep -q ^roundcube$ && grep -q ^skiproundcube=0$ $dir/var/cpanel/cpanel.config && sssh "[ -f /scripts/convert_roundcube_mysql2sqlite ]" && options[20]=on && cmd[8]=`echo "${cmd[8]}\n(7) Source server has roundcube MySQL DB and can be converted"`
	...
	for choice in $choices; do
		case $choice in
			...
			4) addmotd=1;;
			5) dbbackup_schema=1;;
			6) fixperms=1; download_fixperms;;
			7) rcubesqlite=1;;
			*) :;;
		esac
	done
